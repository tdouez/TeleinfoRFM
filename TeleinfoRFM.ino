...
#define RFM_TX_POWER   8   // 5..23 dBm, ne pas dépasser les 9db car la capacité du supercondo (2x0,47F) ne serait pas suffisante. l'arduino rebooterait en permanence
...
unsigned long TIMEOUT_SERIAL_TIC  =  10000; // 10s,  Minimum time between send (in milliseconds). 
...
// ---------------------------------------------------------------- 
boolean search_adress_teleinfo(ValueList *vl_tic, boolean all_tic)
{
    ...
    while (vl_tic->next) {
      ...
      if ( all_tic || ( vl_tic->flags & (TINFO_FLAGS_UPDATED | TINFO_FLAGS_ADDED) )) {
        ...
        if (vl_tic->name && strlen(vl_tic->name) > 2 && vl_tic->value && strlen(vl_tic->value) > 1) {
          ...
          strcat(trame, ";");
          strncat(trame, vl_tic->value, 32);  // limite à 32 caractères pas valeur
          strcat(trame, ";");
...
void NewFrame(ValueList *vl_tic)
{
  ...
  if (!flag_adresse_tic) search_adress_teleinfo(vl_tic, full_tic);
...
void setup()
{
  ...
  Serial.flush();
  Serial.end();
  Serial.begin(MODE_TIC == TINFO_MODE_HISTORIQUE ? 1200 : 9600, SERIAL_7E1);
